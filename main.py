...
# app.add_url_rule('/static/Datasets/<path:filename>', endpoint='Datasets', view_func=app.send_static_file)
# app.add_url_rule('/static/WSI Datasets/<path:filename>', endpoint='WSI', view_func=app.send_static_file)
# app.add_url_rule('/static/Classifications/History/<path:filename>', endpoint='History', view_func=app.send_static_file)
...
# @app.route('/api/classification/image/prediction', methods=['GET'])
# def imagePredictionClassificationAPI():
#   if (session.get("username") is None or not session.get("is_authenticated")):
#     response = {
#       "result"    : None,
#       "is_success": False,
...
#     return response
#
#   basePath = os.path.join(CLASSIFICATIONS_PATH, "Inference")
#   filePath = os.path.join(basePath, request.args.get("target"), request.args.get("image"))
#   print(filePath)
#
#   if (os.path.exists(filePath)):
#     if (os.path.isfile(filePath)):
...
# @app.route('/api/classification/image/history', methods=['GET'])
# def imageHistoryClassificationAPI():
#   if (session.get("username") is None or not session.get("is_authenticated")):
#     response = {
#       "result"    : None,
#       "is_success": False,
...
#   historyPath = os.path.join(CLASSIFICATIONS_PATH, "History", target, "history.json")
#   historyDF = pd.read_json(historyPath)
#   historyPlot = GetTrainingHistoryPlot(historyDF)
#
#   response = {
#     "result"    : historyPlot,
#     "is_success": True,
...
# @app.route('/api/segmentation/image/history', methods=['GET'])
# def imageHistorySegmentationAPI():
#   if (session.get("username") is None or not session.get("is_authenticated")):
#     response = {
#       "result"    : None,
#       "is_success": False,
...
#
#   target = request.args.get("target")
#   historyPath = os.path.join(SEGMENTATIONS_PATH, "History", target, "history.json")
#   historyDF = pd.read_json(historyPath)
#   historyPlot = GetTrainingHistoryPlot(historyDF, metrics=["loss", "accuracy", "DiceCoeff", "IOUCoeff"])
#
#   response = {
#     "result"    : historyPlot,
#     "is_success": True,
...
# allElements = [f for f in sorted(os.listdir(DZI_PATH)) if f.endswith(".dzi")]
# segmentations = sorted(os.listdir(os.path.join(SEGMENTATIONS_PATH, "History")))
#
# noOfElements = len(allElements)
# elements = allElements[fromNumber:fromNumber + stepNumber]
#
# result = []
# for element in elements:
#
#   segmentationsRuns = []
#   elementNoExt = ".".join(element.split(".")[:-1])
#   for segmentFolder in segmentations:
#     configsPath = os.path.join(SEGMENTATIONS_PATH, "History", segmentFolder, "configs.json")
#     f = open(configsPath, mode='r')
#     configs = json.load(f)
#     f.close()
#     if (configs["Whole Slide Image (WSI) File"] == elementNoExt + ".svs"):
#       segmentationsRuns.append(segmentFolder)
#
#   elementPath = os.path.join(DZI_PATH, element)
#   result.append(
#     {
#       "name"         : element,
#       "path"         : element,
#       "segmentations": segmentationsRuns,
#       "modified_at"  : datetime.fromtimestamp(os.path.getmtime(elementPath)).strftime('%Y-%m-%d %H:%M:%S'),
#       "created_at"   : datetime.fromtimestamp(os.path.getctime(elementPath)).strftime('%Y-%m-%d %H:%M:%S'),
#       "accessed_at"  : datetime.fromtimestamp(os.path.getatime(elementPath)).strftime('%Y-%m-%d %H:%M:%S')
#     }
#   )
#
# response = {
#   "result"    : {"elements": result, "count": noOfElements},
#   "is_success": True,
#   "message"   : "Success loading the elements.",
# }
#
...
#
# item = "-".join(request.form['item'].split("-")[3:])
# filePath = os.path.join(DZI_PATH, item)
#
# if (os.path.exists(filePath)):
#   os.remove(filePath)
#   shutil.rmtree(filePath.replace(".dzi", "") + "_files")
#
#   response = {
#     "result"    : None,
#     "is_success": True,
...
#   }
#   return response
# if ('rotation' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Rotation value is not defined.",
#   }
#   return response
#
# if ('widthShift' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Width shift value is not defined.",
#   }
#   return response
#
# if ('heightShift' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Height shift value is not defined.",
#   }
#   return response
#
# if ('zoom' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Zoom value is not defined.",
#   }
#   return response
#
# if ('verticalFlip' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Vertical flip value is not defined.",
#   }
#   return response
#
# if ('horizontalFlip' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Horizontal flip value is not defined.",
#   }
#   return response
#
# if ('shear' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Shear value is not defined.",
#   }
#   return response
#
...
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Brightness value is not defined.",
#   }
#   return response
#
# if ('saturation' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Saturation value is not defined.",
#   }
#   return response
#
# if ('hue' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "Hue value is not defined.",
#   }
#   return response
#
# if ('specific' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "The specific (or random) value is not defined.",
#   }
#   return response
#
# if ('single' not in request.form):
#   response = {
#     "result"    : None,
#     "is_success": False,
#     "message"   : "The single (or multiple) value is not defined.",
#   }
#   return response
...
# while (os.path.exists(filePath)):
#   newFileName = ".".join(filename.split(".")[:-1]) + " (" + str(counter) + ")." + filename.split(".")[-1]
#   filePath = os.path.join(LIVE_AUGMENTATIONS_PATH, newFileName)
#   counter += 1
# filename = newFileName
# filePath = os.path.join(LIVE_AUGMENTATIONS_PATH, filename)

# for field in allFields:
#   if (field == '' or len(field.strip()) <= 0):
#     flash("Please fill all fields. All are required.", "error")
#     return redirect(url_for('classification'))
#
# if (not IsNumber(tlTrainingRatio)):
#   flash("Transfer Learning Training Ratio MUST be a Number.", "error")
#   return redirect(url_for('classification'))
# elif (float(tlTrainingRatio) < 0 or float(tlTrainingRatio) > 1):
#   flash("Transfer Learning Training Ratio MUST be between 0 (inclusive) and 1 (inclusive).", "error")
#   return redirect(url_for('classification'))
#
# if (not IsInt(batchSize)):
#   flash("Batch Size MUST be an Integer.", "error")
#   return redirect(url_for('classification'))
# elif (int(batchSize) <= 0):
#   flash("Batch Size MUST be greater than 0.", "error")
#   return redirect(url_for('classification'))
#
# if (not IsInt(epochs)):
#   flash("Epochs MUST be an Integer.", "error")
#   return redirect(url_for('classification'))
# elif (int(epochs) <= 0):
#   flash("Epochs MUST be a Positive Integer.", "error")
#   return redirect(url_for('classification'))
#
# if (not IsNumber(trainingRatio)):
#   flash("Training Ratio MUST be a Number.", "error")
#   return redirect(url_for('classification'))
# elif (float(trainingRatio) <= 0 or float(trainingRatio) > 1):
#   flash("Training Ratio MUST be a Number between 0 (exclusive) and 1 (inclusive).", "error")
#   return redirect(url_for('classification'))
# histories = []
# for el in os.listdir(os.path.join(CLASSIFICATIONS_PATH, "History")):
#   if os.path.isdir(os.path.join(CLASSIFICATIONS_PATH, "History", el)):
#     configsPath = os.path.join(CLASSIFICATIONS_PATH, "History", el, "configs.json")
...
#
#     if (configs["Success"]):
#       historyPath = os.path.join(CLASSIFICATIONS_PATH, "History", el, "history.json")
#       historyDF = pd.read_json(historyPath)
#       historyCols = historyDF.columns.tolist()
#       historyCols = [el.upper().replace("VAL_", "VAL ") for el in historyCols]
#       historyVals = historyDF.values.tolist()
#       historyVals = np.round(historyVals, 4).tolist()
#
#       evaluationsPath = os.path.join(CLASSIFICATIONS_PATH, "History", el, "evaluations.json")
#       evaluationsDF = pd.read_json(evaluationsPath)
#       evaluationsCols = evaluationsDF.columns.tolist()
#       evaluationsCols = [el.upper().replace("VAL_", "VAL ") for el in evaluationsCols]
#       evaluationsVals = np.array(evaluationsDF.values.tolist())
#       typeIndex = evaluationsCols.index("TYPE")
#       evaluationsCols.pop(typeIndex)
#       evaluationsCols.insert(0, "TYPE")
#       arrangement = list(range(len(evaluationsCols)))
#       arrangement.pop(typeIndex)
#       arrangement.insert(0, typeIndex)
#       evaluationsVals[:, :] = evaluationsVals[:, arrangement]
#       evaluationsVals[:, 1:] = np.round(evaluationsVals[:, 1:].astype("float64"), 4)
#       predictionsPath = os.path.join(CLASSIFICATIONS_PATH, "History", el, "predictions.csv")
#       if (os.path.exists(predictionsPath)):
#         predictionsDF = pd.read_csv(predictionsPath)
#         predictionsCols = predictionsDF.columns.tolist()
#         predictionsIDs = predictionsDF["Image"].tolist()
#         predictionsVals = predictionsDF.values.tolist()
#       else:
#         predictionsCols = []
#         predictionsIDs = []
#         predictionsVals = []
...
#           "id"         : el,
#           "configs"    : configs,
#           "history"    : {"cols": historyCols, "vals": historyVals},
#           "evaluations": {"cols": evaluationsCols, "vals": evaluationsVals},
#           "predictions": {"cols": predictionsCols, "vals": predictionsVals,
#                           "ids" : predictionsIDs} if os.path.exists(
#             predictionsPath
#           ) else None,
...
# @app.route('/api/classification/clear', methods=['POST'])
# def clearHistoryClassificationAPI():
#   if (session.get("username") is None or not session.get("is_authenticated")):
#     response = {
#       "result"    : None,
#       "is_success": False,
...
#     return response
#
#   shutil.rmtree(CLASSIFICATIONS_PATH)
#
#   response = {
#     "result"    : None,
#     "is_success": True,
#     "message"   : "history is cleared successfully.",
#   }
#   return response
# histories = []
#
#     for el in os.listdir(os.path.join(SEGMENTATIONS_PATH, "History")):
#       if os.path.isdir(os.path.join(SEGMENTATIONS_PATH, "History", el)):
#         configsPath = os.path.join(SEGMENTATIONS_PATH, "History", el, "configs.json")
#         if (not os.path.exists(configsPath)):
#           continue
#
#         f = open(configsPath, mode='r')
...
#         f.close()
#
#         historyPath = os.path.join(SEGMENTATIONS_PATH, "History", el, "history.json")
#         historyDF = pd.read_json(historyPath)
#         historyCols = historyDF.columns.tolist()
#         historyCols = [el.upper().replace("VAL_", "VAL ") for el in historyCols]
#         historyVals = historyDF.values.tolist()
#         historyVals = np.round(historyVals, 4).tolist()
#
#         evaluationsPath = os.path.join(SEGMENTATIONS_PATH, "History", el, "evaluations.json")
#         evaluationsDF = pd.read_json(evaluationsPath)
#         evaluationsCols = evaluationsDF.columns.tolist()
#         evaluationsCols = [el.upper().replace("VAL_", "VAL ") for el in evaluationsCols]
#         evaluationsVals = np.array(evaluationsDF.values.tolist())
...
#         evaluationsVals[:, 1:] = np.round(evaluationsVals[:, 1:].astype("float64"), 4)
#
#         predictionsPath = os.path.join(SEGMENTATIONS_PATH, "History", el, "predictions.csv")
#         if (os.path.exists(predictionsPath)):
#           predictionsDF = pd.read_csv(predictionsPath)
#           predictionsCols = predictionsDF.columns.tolist()
#           predictionsIDs = predictionsDF["Image"].tolist()
...
#             "history"    : {"cols": historyCols, "vals": historyVals},
#             "evaluations": {"cols": evaluationsCols, "vals": evaluationsVals},
#             "predictions": {"cols": predictionsCols, "vals": predictionsVals, "ids": predictionsIDs} if os.path.exists(
#               predictionsPath
#             ) else None,
#             "success"    : configs["Success"],
#           }
#         )
# if (qupathProject.filename == ''):
#   flash("QuPath project is not selected.", "error")
#   return redirect(url_for('segmentation'))
#
# ext = qupathProject.filename.split('.')[-1].lower()
# if (ext not in QUPATH_COMP_TYPES):
#   flash("QuPath project extension is not supported.", "error")
#   return redirect(url_for('segmentation'))
#
# for field in allFields:
#   if (field == '' or len(field.strip()) <= 0):
...
#     return redirect(url_for('segmentation'))
#
# if (not os.path.exists(os.path.join(WSI_PATH, wsiFile))):
#   flash("WSI File does not exist.", "error")
#   return redirect(url_for('segmentation'))
#
# if (not IsInt(batchSize)):
#   flash("Batch Size MUST be an Integer.", "error")
...
#   flash("Training Ratio MUST be a Number between 0 (exclusive) and 1 (inclusive).", "error")
#   return redirect(url_for('segmentation'))
# @app.route('/api/segmentation/inference', methods=['POST'])
# def inferenceSegmentationAPI():
#   global stopQueueThread, queueThread
#
#   if (session.get("username") is None or not session.get("is_authenticated")):
#     response = {
...
#   if (file):
#     filename = secure_filename(file.filename)
#     basePath = os.path.join(SEGMENTATIONS_PATH, "Inference")
#
#     folderPath = os.path.join(basePath, request.form.get("target"))
...
#     while (os.path.exists(filePath)):
#       newFileName = ".".join(filename.split(".")[:-1]) + " (" + str(counter) + ")." + filename.split(".")[-1]
#       filePath = os.path.join(folderPath, newFileName)
#       counter += 1
#
#     filename = newFileName
#     filePath = os.path.join(folderPath, filename)
#     file.save(filePath)
#     workingQueue.put(("SEG_WEK_INF", request.form.get("target"), filename))
#
#     if (stopQueueThread):
#       stopQueueThread = False
#       del queueThread
...
#       "result"    : None,
#       "is_success": True,
#       "message"   : "Segmentation inference process is established successfully.",
#     }
#
#     return response
#
